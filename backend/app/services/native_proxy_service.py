...
class NativeProxyService:
    ...
    async def _build_request(
        self, request_id: str, provider_name: str, native_path: str, method: str, request: Request
    ) -> NativeProxyRequest:
    ...
    async def forward(
        ...
        if not provider or "native_proxy" not in (provider.protocol_modes or []):
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Native provider is unavailable")
        ...
        native_request = await self._build_request(request_id, provider_name, native_path, method, request)
        ...
        try:
            ...
            await self.usage.record(
                ...
                raw_request_body=self._json_or_text(native_request.body, request.headers.get("content-type"))
            ...
            try:
                async with asyncio.timeout(self.settings.native_stream_max_seconds):
                    async for chunk in adapter.stream_forward(provider, native_request):
                        if time.perf_counter() - stream_started > self.settings.native_stream_max_seconds:
